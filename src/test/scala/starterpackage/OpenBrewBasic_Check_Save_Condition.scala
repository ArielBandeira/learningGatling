...
class OpenBrewBasic_Check_Save_Condition extends Simulation {
  ...
  val scn = scenario("find_breweries")
    ...
    .doIfOrElse(session => session("ResponseBody").as[String].contains("banjo-dfbrewing-fayetteville") && session("ResponseBody").as[String].contains("banjo-brewing-fayetteville")) {
